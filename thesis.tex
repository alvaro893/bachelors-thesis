...
Tero Nurminen, TKI-vastaava yliopettaja \newline
}
\def\metropoliakeywords {IoT, IR, sensor, cloud, full-stack, communication channel, websocket, http}
\date{\today}
...
\newglossaryentry{ws}{
	name=Websocket, 
	description={Application protocol build on top of \gls{tcp}}
}
\newglossaryentry{github}{
	name=GitHub, 
	description={Hosting site for \gls{git} repositories}
}
\newglossaryentry{git}{
	name=Git, 
	description={Control version software}
}
\newglossaryentry{docker}{
	name=Docker, 
	description={Software container platform}
}
...
\begin{tabular}{ | p{4,7cm} | p{10,3cm} |}
  ...
  \paragraph{}	
 The objective of this thesis was to develop a bidirectional communication system for \gls{ir} cameras using \gls{iot} modules to connect them to mobile networks through \gls{lte} and creating all the software solutions to accomplish it which is mainly a sensor-side, client-side and a server-side application, using also cloud based tools in the server-side
  \paragraph{}
  One of the key points of \gls{ir} cameras is the respect of privacy, Unlike normal cameras generally people cannot be recognized. Applications of this in health care and well-being are discussed on chapter~\ref{sec:discussion}, these application are mostly related to monitoring patients e.g. trigger alarm when a patient does not move for a while, control the number of people in a room, etc.
  \paragraph{}
...
Although with \gls{iot} it comes the process of defining a communication channel between the \gls{iot} devices, services, clients, databases, etc. In order to manipulate, visualize, store and distribute data from them. On this thesis it is explained the steps of how to develop software solutions to these topics and other alternatives that were tried, which ones and why were implemented at the end along tools and technologies which were used to carry out the project.

In Finland the costs in elder and mental health care are becoming more expensive. Health care entities in Finland may decrease their costs by search for new ideas in the field of IT. In  the discussion chapter some ideas to use \gls{ir} sensors and the communication channel are explained.
...
The thesis originates in a collaboration with a startup called Levitezer (\url{http://www.levitezer.com/}) which provides the \gls{ir} sensors. A company that develops gimbals and controllers for cameras for image stabilization among other projects~\cite{levitezer}.

Even though this is not a health care company, the sensors and methods here explained could be used but no limited to Hospitals, sanatoriums, clinics, nursing homes, etc. And right now Levitezer is trying to expand their business.
%% TODO: mention nokia in introduction?
The methods and software developed are being used to create an \gls{api} that connect \gls{ir} sensors together.
...
\begin{enumerate}
\item full working communication Chanel using modern cloud technologies, client-side pieces of software able to read sensors; displaying data in form of images and being able of sending commands to modify sensor behavior, all in bidirectional full duplex communication context.
\item proposals to use it on health care institutions.
\end{enumerate}
...
To run programs and script automatically from booting it is possible to add systemd based services which are in most of Linux distributions. For the purposes of this sensor reader it is needed at least 2 services. One to keep alive the network connection alive. Other to keep the reader program constantly running.

\subsubsection{remote communication}
In order to get access to the raspberry when it is out of reach, connections to it through Internet using \gls{ssh} are the most common way
...
% TODO: consider deleting some code in appendices.
% TODO: lte module appendix.
\section{Configuration}
\putcode{configure_at_commands}{Configure and test an Internet connection}[style=styleprogrammingappendix]$
