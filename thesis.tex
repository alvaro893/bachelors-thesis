...
\newline
 On the project there is described different methods to build this communication channel from a full stack point of view. The methods are group on different sides depending on the point of view of each element of the communication channel which are sensor, server and client side. The described sensor-side methods are different pieces of hardware to connect the sensor to the network: an Android smartphone used as a gateway, an LTE module and a Raspberry Pi with a LTE dongle. The server-side methods are different languages and frameworks to control how clients and sensor are connected to the cloud through a web API. The client-side methods are different ways to interpret the data which comes from the cloud and therefore the sensors. There is also description about other methods for testing, cloud, control version and document preparation system \newline
\newline
...
%In the discussion chapter some ideas to use thermal or \gls{ir} sensors and the communication channel are explained~\cite{agingeuropa}
